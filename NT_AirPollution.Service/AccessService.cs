    ...
    public class AccessService : BaseService
    {
                ...
                using (var cn = new OleDbConnection(accessConnStr))
                {
                    cn.Execute(@"
                        ...
                        new
                        {
                            ...
                            S_NAME = form.S_NAME,
                            S_G_NO = string.IsNullOrEmpty(form.S_G_NO) ? null : form.S_G_NO,
                            S_ADDR1 = form.S_ADDR1,
                            S_ADDR2 = form.S_ADDR2,
                            S_TEL = form.S_TEL,
                            S_B_NAM = form.S_B_NAM,
                            S_B_TIT = form.S_B_TIT,
                            S_B_ID = string.IsNullOrEmpty(form.S_B_ID) ? null : form.S_B_ID,
                            S_B_BDATE = form.S_B_BDATE,
                            S_C_NAM = form.S_C_NAM,
                            S_C_TIT = form.S_C_TIT,
                            S_C_ID = string.IsNullOrEmpty(form.S_C_ID) ? null : form.S_C_ID,
                            S_C_ADDR = form.S_C_ADDR,
                            S_C_TEL = form.S_C_TEL,
                            R_NAME = form.R_NAME,
                            R_G_NO = string.IsNullOrEmpty(form.R_G_NO) ? null : form.R_G_NO,
                            R_ADDR1 = form.R_ADDR1,
                            R_ADDR2 = form.R_ADDR2,
                            R_TEL = form.R_TEL,
                            R_B_NAM = form.R_B_NAM,
                            R_B_TIT = form.R_B_TIT,
                            R_B_ID = string.IsNullOrEmpty(form.R_B_ID) ? null : form.R_B_ID,
                ...
                using (var cn = new OleDbConnection(accessConnStr))
                {
                    ...
                    cn.Execute(@"
                        ...
                        new
                        {
                            ...
                            B_DATE = formB.B_DATE,
                            E_DATE = formB.E_DATE,
                            B_YEAR = Math.Round((applyWorkDays - downDays) / 365, 2, MidpointRounding.AwayFromZero),
                            S_AMT = formB.S_AMT,
        ...
        public string GetFLNo(string pdate)
        {
            try
            {
                ...
                using (var cn = new OleDbConnection(accessConnStr))
                {
                    ...
                    string flnoStr = result?.FLNO;

                    // 檢查是否為空，且長度是否足夠截取後3碼
                    if (!string.IsNullOrEmpty(flnoStr) && flnoStr.Length >= 3)
                    {
                        string last3 = flnoStr.Substring(flnoStr.Length - 3, 3);
                        if (int.TryParse(last3, out int parsedNo))
                        {
                            no = parsedNo + 1;
                        }
                    }
        ...
        public bool AddABUDF_1(ABUDF_1 abudf_1)
        {
            try
            {
                ...
                {
                    // 檢查 ABUDF_1 是否已存在
                    var isExist = cn.ExecuteScalar<int>(@"
                        SELECT COUNT(1) FROM ABUDF_1 WHERE [C_NO]=@C_NO AND [SER_NO]=@SER_NO AND [P_TIME]=@P_TIME",
                        new
                        {
                            C_NO = abudf_1.C_NO,
                            SER_NO = abudf_1.SER_NO,
                            P_TIME = abudf_1.P_TIME
                        }, commandTimeout: 180) > 0;
                    ...
                    else
                    {
                        ...
                        cn.Execute(@"
                                INSERT INTO ABUDF_1 (
                                    [C_NO], [SER_NO], [P_TIME], [P_DATE], [E_DATE], 
                                    [FLNO], [F_AMT], [B_AMT], [KEYIN], [C_DATE], [M_DATE]
                                )
                                VALUES (
                                    @C_NO, @SER_NO, @P_TIME, @P_DATE, @E_DATE, 
                                    @FLNO, @F_AMT, @B_AMT, @KEYIN, @C_DATE, @M_DATE
            ...
            {
                Logger.Error(ex.StackTrace);
                throw; // 建議直接 throw; 保留原本的堆疊資訊，而非 throw ex;
            }
        }
        ...
        /// </summary>
        /// <param name="abudf_1"></param>
        /// <param name="userSelectFLNO">user選擇的銷帳編號</param>
        /// <returns></returns>
        public bool UpdateABUDF_1(ABUDF_1 abudf_1, string userSelectFLNO)
        {
            try
            {
#if !DEBUG
